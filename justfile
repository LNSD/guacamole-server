...
meson-setup *EXTRA_FLAGS:
    meson setup {{build_dir}} {{EXTRA_FLAGS}}
...
# Format meson files (meson format)
meson-format:
    meson format --editor-config --inplace --recursive

alias meson-fmt := meson-format

# Check meson file format (meson format --check-only)
meson-check-format:
    meson format --editor-config --check-only --recursive
